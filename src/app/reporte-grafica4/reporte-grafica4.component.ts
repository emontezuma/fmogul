...
@Component({
  selector: 'app-reporte-grafica4',
  templateUrl: './reporte-grafica4.component.html',
  styleUrls: ['./reporte-grafica4.component.css']
})
export class ReporteGrafica4Component implements OnInit {
...
constructor( public snackBar: MatSnackBar, public dialog: MatDialog, private gestionBD: GestionApisService, private comunicacion: ComunicacionService ) { 
  ...
  ngOnInit() {     
    ...
    this.comunicacion.llamadasVistaGrafica.emit(21);
    };
    ...
    graficar(avance: number)
    {
      ...
      if (this.graficaActual == 0) {
        this.graficaActual = 13;
      }
      else if (this.graficaActual > 13) {
        this.graficaActual = 1;
      }
      this.comunicacion.aGrafica(this.graficaActual + 200);
      ...
      this.gestionBD.consultasBD(campos).subscribe((data: any)=>{
        let campos2 = {accion: 4, usuario: this.comunicacion.rUsuario().id, grafica: this.graficaActual + 200};
        this.gestionBD.consultasBD(campos2).subscribe((data: any)=>{
          if (!data) {
            let campos3 = {accion: 4, usuario: 0, grafica: this.graficaActual + 200};
    ...
    grafico()
    {
      ...
      buscarGrafica()
      {
        //this.comunicacion.graficando.emit(true);
            let campos = {accion: 1900 + this.graficaActual, desde: this.fechaDesde, hasta: this.fechaHasta,  filtroadicional: this.filtroAdicional};
            this.gestionBD.consultasBD(campos).subscribe((data: GraficaMTTR[])=>{
              this.graficamttr = data;
              if (data.length==0) {
                this.hayGrafico = false;
                this.comunicacion.mostrarEstado.emit("Gráfica "  + this.graficaActual + " de 13" + this.estaFiltrado);
...
continuarGrafico() 
{
      ...
      exportarDatos()
      {
        ...
        exportCSV = exportCSV + '"Numero de fallas (frecencia)",';
        exportCSV = exportCSV + '"Tiempo en horas",';
        exportCSV = exportCSV + '"Porcentaje",';
        exportCSV = exportCSV + '"Acumulado (PCT)",' + "\r\n";
        let lineas: number = 0;
        let totalFalla: number = 0;
        let totalHoras: number = 0;
        this.graficamttr.forEach((elementos: GraficaMTTR, index) => {
          lineas=lineas+1
          totalHoras = totalHoras + (elementos.totalf / this.totalMTTR * 100)
          ...
          exportCSV = exportCSV + '"' + (elementos.totalf / this.totalMTTR * 100).toFixed(4) + '",';
          exportCSV = exportCSV + '"' + totalHoras.toFixed(4) + '",'  + "\r\n";
