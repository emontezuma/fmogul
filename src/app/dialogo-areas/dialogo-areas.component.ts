...
@Component({
  selector: 'app-dialogo-areas',
  templateUrl: './dialogo-areas.component.html',
  styleUrls: ['./dialogo-areas.component.css']
})
export class DialogoAreasComponent implements OnInit {
  ...
  secuencia: string = "";
  colorhexa: string = "";
  imagen: string = "./assets/icons/areas.svg";
  modificador: string = "";
  creador: string = "";
  fcreacion: string = "";
  ...
  constructor(public dialogRef: MatDialogRef<DialogoAreasComponent>, @Inject(MAT_DIALOG_DATA) public datos: any, public dialog: MatDialog, private comunicacion: ComunicacionService, private gestionBD: GestionApisService) {
    this.idParo = '' + datos.id;
    if (datos.id > 0) 
    {
      let consulta1 = "SELECT areas.*, a.nombre as tcreador, b.nombre as tmodificador FROM areas LEFT JOIN usuarios AS a ON areas.creado = a.id LEFT JOIN usuarios AS b ON areas.modificado = b.id WHERE areas.id = " + this.idParo;
...
eliminar() {
  const dialogRef = this.dialog.open(DialogoComponent, {
    width: '570px', height: '270px', data: { botones: 2, cabecera: 'INACTIVAR ÁREA DE ATENCIÓN', contenido: 'Esta acción inactivará esta área y no estará disponible en el sistema. ¿Está seguro de esta acción?', boton1: 'Si, la inactivaré', boton2: 'No, lo pensaré mejor', tipoMensaje: 'NORMAL' }
  });
  dialogRef.afterClosed().subscribe(result => {
    ...
    {
      let consulta1 = "update areas set estatus = 'I' WHERE id = " + this.idParo;
...
validar(){
  ...
  if (!this.hayError1 && !this.hayError2)
  {
    ...
    let consulta0 = "";
    let consulta1 = "SELECT max(secuencia) as maxid from areas";
      let camposcab={accion: 50100, consulta: consulta1};  
      this.gestionBD.consultasBD(camposcab).subscribe((data) =>{
      ...
      if (this.idParo=="0") {
        consulta1 = "insert into areas (secuencia, referencia, nombre, imagen, colorhexa, prefijo, estatus, creacion, modificacion, creado, modificado) VALUES (" + (+data.maxid + 1) + ", '" + this.referencia + "', '" + this.nombre +  "', '" + this.imagen + "', '" + this.colorhexa + "', '" + this.prefijo + "', 'A', '" + fecha + "', '" + fecha + "', " + usuario + ", " + usuario + ");"
      }
      else {
        consulta1 = "update areas set estatus = '" + (this.estatus ? "A" : "I") + "', nombre = '" + this.nombre + "', prefijo = '" + this.prefijo + "', referencia = '" + this.referencia + "', colorhexa = '" + this.colorhexa + "', imagen = '" + this.imagen + "', modificado = " + usuario + ", modificacion = '" + fecha + "' WHERE id = " + this.idParo;
        tiporeg = false;
      }
      ...
      this.gestionBD.escribirBD(camposcab).subscribe((data2: any) =>{
          if (!tiporeg && this.secuencia != this.secuenciaantes && this.secuencia!="0") 
          {
            if (+this.secuencia>+data.maxid) {
              consulta1 = "update areas set secuencia = " + data.maxid + " WHERE id = " + this.idParo;
              consulta0 = "update areas set secuencia = secuencia - 1  WHERE secuencia <= " + this.secuencia + " and secuencia > " + this.secuenciaantes + " AND id <> " + this.idParo;  
            }
            else
            {
              ...
              consulta1 = "update areas set secuencia = " + this.secuencia + " WHERE id = " + this.idParo;
            }
            camposcab={accion: 50030, consulta: consulta1};  
              this.gestionBD.escribirBD(camposcab).subscribe((data: any) =>{
