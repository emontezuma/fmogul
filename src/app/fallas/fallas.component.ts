...
export class FallasComponent implements AfterContentInit {
  ...
  constructor( private router: Router, public dialog: MatDialog, private gestionBD: GestionApisService, private comunicacion: ComunicacionService, private observableMedia: ObservableMedia ) { 
    ...
    this.comunicacion.mostrarNota.emit("Fallas registrados en el sistema");
  ...
  filtrarmiArreglo() {
    ...
    return this.arrFiltrado.filter(datos =>
      ...
      ||
      datos.narea.toLowerCase().indexOf(this.filtrarPor.toLowerCase()) !== -1
      ||
  ...
  llenarPantalla(orden) {
    ...
    //Recupera la tabla de celulas
      ...
      let consulta2 = "SELECT c.*, a.nombre AS tcreador, b.nombre AS tmodificador, d.prefijo AS narea, e.prefijo AS ncelula, f.prefijo AS nclas1, g.prefijo AS nclas2, 0 as tmaquinas FROM fallas AS c LEFT JOIN usuarios AS a ON c.creado = a.id LEFT JOIN usuarios AS b ON c.modificado = b.id LEFT JOIN areas AS d ON c.area = d.id LEFT JOIN celulas AS e ON c.celula = e.id LEFT JOIN generales AS f ON c.clasificacion1 = f.id LEFT JOIN generales AS g ON c.clasificacion2 = g.id ORDER BY" + cadOrden ;
      let camposcab={accion: 50300, consulta: consulta2};  
      this.gestionBD.consultasBD(camposcab).subscribe((data: any []) =>{
        data.forEach((elemento) => {
          if (!elemento.narea){elemento.narea="Cualquiera"};
          if (!elemento.ncelula){elemento.ncelula="Cualquiera"};
  ...
  contarRegs() {
    ...
    if (this.celulas.length == 1) {
      this.comunicacion.mostrarEstado.emit(this.celulas.length + " falla" + filtrado);    
    }
    else if (this.celulas.length > 0 ) {
      this.comunicacion.mostrarEstado.emit(this.celulas.length + " fallas" + filtrado);
    }
  else{
  ...
  paro(id: number) {
    const dialogRef = this.dialog.open(DialogoFallasComponent, {
  ...
  agregarParo() {
   ...
   descargar() {
    let exportCSV: string = "";
        ...
        exportCSV = "Listado de fallas registradas en el sistema: " + this.comunicacion.convertirFecha(1, '' , "yyyy/MM/dd hh:mm a") + "\r\n";
        exportCSV = exportCSV + '"ID de la falla (autonumerico)",';
        ...
        exportCSV = exportCSV + '"ID del area asociada ",';
        exportCSV = exportCSV + '"Prefijo del area asociada ",';
        ...
        exportCSV = exportCSV + '"Prefijo de la clasificacion (2) asociada ",';
        ...
        this.celulas.forEach((elementos) => {
          ...
          exportCSV = exportCSV + '"' + elementos.narea  + '",';
