...
@Component({
  selector: 'app-reporte-grafica3',
  templateUrl: './reporte-grafica3.component.html',
  styleUrls: ['./reporte-grafica3.component.css']
})
export class ReporteGrafica3Component implements OnInit {
...
constructor( public snackBar: MatSnackBar, public dialog: MatDialog, private gestionBD: GestionApisService, private comunicacion: ComunicacionService ) { 
  ...
  this.comunicacion.cambiarGrafica.subscribe((data: number)=>{
    if (data==11)
    {this.graficar(1)}
    else if (data==-11)
    {this.graficar(-1)}
  });
  this.comunicacion.guardarGrafica.subscribe((data: number)=>{
    if (data==11) {this.exportarImagen()}
  });
  this.comunicacion.iniGrafico.subscribe((data: number)=>{
    if (data==11)
    this.graficaActual=0;
  });
  
  this.comunicacion.exportarGrafica.subscribe((data: number)=>{
    if (data==11) {this.exportarDatos()}
  });
  this.comunicacion.aplicarFiltro.subscribe((data: number)=>{
    if (data==11) {this.graficar(0)};
  });
}
  ...
  ngOnInit() {     
    this.pantallaAncho = window.innerWidth;
      ...
      this.comunicacion.llamadasVistaGrafica.emit(11);
    };
    ...
    graficar(avance: number)
    {
      ...
      this.graficaActual = this.graficaActual + avance;
      if (this.graficaActual == 0) {
        this.graficaActual = 12;
      }
      else if (this.graficaActual > 12) {
        this.graficaActual = 1;
      }
      this.comunicacion.aGrafica(this.graficaActual + 100);
      ...
      this.gestionBD.consultasBD(campos).subscribe((data: any)=>{
        let campos2 = {accion: 4, usuario: this.comunicacion.rUsuario().id, grafica: this.graficaActual + 100};
        this.gestionBD.consultasBD(campos2).subscribe((data: any)=>{
          if (!data) {
            let campos3 = {accion: 4, usuario: 0, grafica: this.graficaActual + 100};
      ...
      llenarDatos(data: any) {
          ...
          if (data.alto==0)
          {
            this.alto = this.pantallaAlto * 0.75;
          }
          else
          {
            this.alto = +data.alto;
    ...
    grafico()
    {
      ...
      let consultaSQL = "SELECT * from consultas_cab WHERE id = " + this.comunicacion.recuperConsultaActual()  ;
        let camposcab={accion: 50100, consulta: consultaSQL};  
          this.gestionBD.consultasBD(camposcab).subscribe((data: any) =>{
            this.estaFiltrado= " (se está aplicando un filtro)"
            if (data.fecha == 7) {
      ...
      buscarGrafica()
      {
        //this.comunicacion.graficando.emit(true);  
            ...
            this.gestionBD.consultasBD(campos).subscribe((data: GraficaMTTR[])=>{
              ...
              if (data.length==0) {
                ...
                this.hayGrafico = false;
                this.comunicacion.mostrarEstado.emit("Gráfica "  + this.graficaActual + " de 12" + this.estaFiltrado);
...
acumularMes()
{
  if (this.verMesAnterior=="S") {
    ...
    let camposant = {accion: 800, desde: fechaTrabajoINI2, hasta: fechaTrabajoFIN2,  filtroadicional: this.filtroAdicional};
    this.gestionBD.consultasBD(camposant).subscribe((anterior: any)=>{
      if (anterior[0].mtbfc!=null) {
        this.mttrmensual = anterior[0].mtbfc;
        this.mttrmensualstd = anterior[0].mtbfs;
      }
      this.acumularAnyo();
    });
  }
...
acumularAnyo()
{
  if (this.verAnyoAnterior=="S") {
    ...
    this.gestionBD.consultasBD(camposant).subscribe((anterior: any)=>{
      if (anterior[0].mtbfc!=null) {
        this.mttranual= anterior[0].mtbfc;
        this.mttranualstd = anterior[0].mtbfs;
      }
...
acumularMesComp()
{
  if (this.verMesAnteriorComp=="S") {
    ...
    this.gestionBD.consultasBD(camposant).subscribe((anterior: any)=>{
      if (anterior[0].mtbfc!=null) {
        this.mttrmescomp = anterior[0].mtbfc;
        this.mttrmescompstd = anterior[0].mtbfs;
      }
      this.acumularYTD();
    });
  }
...
acumularYTD()
{
  if (this.verYTD=="S") {
    ...
    this.gestionBD.consultasBD(camposant).subscribe((anterior: any)=>{
      if (anterior[0].mtbfc!=null) {
        this.mttrytd = anterior[0].mtbfc;
        this.mttrytdstd = anterior[0].mtbfs;
      }
      this.acumularMTD();
    });
  }
...
acumularMTD()
{
  if (this.verMTD=="S") {
    ...
    this.gestionBD.consultasBD(camposant).subscribe((anterior: any)=>{
      if (anterior[0].mtbfc!=null) {
        this.mttrmtd = anterior[0].mtbfc;
        this.mttrmtdstd = anterior[0].mtbfs;
      }
...
continuarGrafico() 
{
      ...
      this.bborderColor = [];
      this.bbackgroundColor = [];
      this.graficamttr.sort(this.compare);
      let eliminar: number = -1;
      this.totalMTTR = 0;
      this.graficamttr.forEach((elementos: GraficaMTTR, index) => {
        if (elementos.pref) {
          this.estandar = elementos.mtbfs;
          this.totalMTTR = this.totalMTTR + +elementos.mtbfc *1;
          //alert("MTTR solo: " + elementos.mttrc + " acumulado: " + this.totalMTTR + " maximo PCT: " + this.maximoPCT)
          this.valores.push({nReal: elementos.mtbfc, nEstandar: elementos.mtbfs, nEtiqueta: elementos.pref});
      ...
      if (+this.maximoPCT > 0 || (+this.maximoBarras > 0 && this.graficamttr.length > this.maximoBarras )) {
        ...
        this.graficamttr.forEach((elementos, index) => {
          acumuladoMTTR = acumuladoMTTR + elementos.mtbfc*1; 
      ...
      //Mes pasado
        ...
        this.valores.unshift({nReal: this.ultimo.mtbfc, nEstandar: this.ultimo.mtbfs, nEtiqueta: 'PLANTA'});
        //
      ...
      exportarDatos()
      {
        ...
        this.graficamttr.forEach((elementos: GraficaMTTR, index) => {
          ...
          exportCSV = exportCSV + '"' + elementos.mttrs + '",';
          exportCSV = exportCSV + '"' + elementos.mtbfs + '",';
          exportCSV = exportCSV + '"' + elementos.totalf + '",';
          exportCSV = exportCSV + '"' + elementos.totalr + '",';
          exportCSV = exportCSV + '"' + (elementos.tparos / 3600).toFixed(2) + '",';
          exportCSV = exportCSV + '"' + (elementos.tdisp / 3600).toFixed(2) + '",';
          exportCSV = exportCSV + '"' + elementos.mttrc + '",';
          exportCSV = exportCSV + '"' + elementos.mtbfc + '",' + "\r\n";
          totalFalla = totalFalla + +elementos.totalf;
          totalHoras = totalHoras + +elementos.totalr;
        })
        exportCSV = exportCSV + '"Total registro(s): "' + lineas + ",,,," + totalFalla + "," + totalHoras + ",,," + this.ultimo.mttrc + "," + this.ultimo.mtbfc
