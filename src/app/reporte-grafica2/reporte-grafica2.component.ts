...
@Component({
  selector: 'app-reporte-grafica2',
  templateUrl: './reporte-grafica2.component.html',
  styleUrls: ['./reporte-grafica2.component.css']
})
export class ReporteGrafica2Component implements OnInit {
...
constructor( public snackBar: MatSnackBar, public dialog: MatDialog, private gestionBD: GestionApisService, private comunicacion: ComunicacionService ) { 
  ...
  ngOnInit() {     
      ...
      this.comunicacion.llamadasVistaGrafica.emit(1);
    };
    ...
    graficar(avance: number)
    {
      ...
      if (this.graficaActual == 0) {
        this.graficaActual = 15;
      }
      else if (this.graficaActual > 15) {
...
buscarGrafica()
{
  //this.comunicacion.graficando.emit(true);
      ...
      this.gestionBD.consultasBD(campos).subscribe((data: GraficaMTTR[])=>{
        ...
        if (data.length==0) {
          this.hayGrafico = false;
          this.comunicacion.mostrarEstado.emit("Gráfica "  + this.graficaActual + " de 15" + this.estaFiltrado);
...
continuarGrafico() 
{
      ...
      //Mes pasado
        ...
        this.chart._setOption("valueAxis",
            ...
            dashStyle:"dash",
            label: 
                  { text: this.textoMTTResperado + ' ' + this.estandar + 'h', font: { size: this.f4, weight: 600, bold: "true"}}
          }],
              valueType: "numeric", 
                title: 
                    { text: this.textoY, 
      ...
      exportarDatos()
      {
        ...
        exportCSV = exportCSV + (this.graficaActual<14 ? '"Total horas de reparacion",' : '"MTTR calculado",');
        exportCSV = exportCSV + '"Total horas (paros planeados)",';
        exportCSV = exportCSV + '"Total horas disponibles",';
        exportCSV = exportCSV + (this.graficaActual<14 ? '"MTTR calculado,"' : '"Total horas de reparacion",');
        ...
        this.graficamttr.forEach((elementos: GraficaMTTR, index) => {
          ...
          exportCSV = exportCSV + '"' + (this.graficaActual<14 ? elementos.totalr : elementos.mttrc) + '",';
          exportCSV = exportCSV + '"' + (elementos.tparos / 3600).toFixed(2) + '",';
          exportCSV = exportCSV + '"' + (elementos.tdisp / 3600).toFixed(2) + '",';
          exportCSV = exportCSV + '"' + (this.graficaActual<14 ? elementos.mttrc : elementos.totalr) + '",';
        ...
        {
          exportCSV = exportCSV + '"Total registro(s): "' + lineas + ",,,," + totalFalla + "," + this.ultimo.mttrc + ",,," + totalHoras + "," + this.ultimo.mtbfc
        }
