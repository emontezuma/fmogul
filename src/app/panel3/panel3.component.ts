...
export class Panel3Component implements AfterContentInit {
  ...
  constructor( private gestionBD: GestionApisService, private router: Router, private comunicacion: ComunicacionService, private observableMedia: ObservableMedia ) { 
    ...
    this.llenarPantalla();
    setTimeout(() => {
      this.comunicacion.panelActual(11)
    }, 500);
    this.comunicacion.refrescarCelulasPanel.subscribe((data: any)=>{
      if (data == 11) 
      {this.llenarPantalla()}});
    this.comunicacion.aplicarFiltroMaquinaPanel.subscribe((data: string)=>{
  ...
  continuarLlenado() {
    ...
    let campos={accion: 1811, desde: fechaDesde, hasta: fechaHasta, filtroadicional: "WHERE a.estatus = 'A'"};
    ...
    this.gestionBD.consultasBD(campos).subscribe((data: any[])=>{
      data.forEach((registro, index) => {
        ...
        campos={accion: 1811, desde: fechaDesde, hasta: fechaHasta, filtroadicional: "WHERE a.id = " + registro.id};
        this.gestionBD.consultasBD(campos).subscribe((masDatos: any )=>{
       
          let campos2={accion: 1802, filtro: "AND maquina = " + registro.id};
          this.gestionBD.consultasBD(campos2).subscribe((dataActual: any[])=>{

            campos2 = {accion: 1803, filtro: "SELECT maquinas.*, celulas.prefijo as cprefijo, celulas.id as cid, celulas.referencia as creferencia FROM maquinas LEFT JOIN celulas ON maquinas.celula = celulas.id WHERE maquinas.id = "  + registro.id};
              this.gestionBD.consultasBD(campos2).subscribe((data2: any)=>{
               ...
               else
               {
                ...
                pcid = data2.cid;
                pcprefijo = data2.cprefijo;
                pcreferencia = data2.creferencia;
  ...
  contarRegs() {
    if (this.comunicacion.rpanelActual()==11)
    {
    let filtrado = "";
    ...
    this.comunicacion.mostrarEstado.emit("No hay máquinas" + alarmadas + filtrado);
    }
  ...
  ngAfterContentInit() {
    ...
    this.comunicacion.llamadasVistaPanel.emit(11);
  }
  ...
  cronometro() {
    ...
    this.celulas.forEach((miReporte, index) => {
      let campos2={accion: 1802, filtro: "AND maquina = " + miReporte.id};
  ...
  descargar() {
    let exportCSV: string = "";
        ...
        exportCSV = exportCSV + '"ID de maquina",';
        ...
        exportCSV = exportCSV + '"Referencia de la celula asociada",';
        ...
        this.celulas.forEach((elementos: datosPantalla, index) => {
          ...
          exportCSV = exportCSV + '"' + elementos.cid + '",';
          exportCSV = exportCSV + '"' + elementos.creferencia + '",';
          exportCSV = exportCSV + '"' + elementos.cprefijo + '",';
        ...
        link.download = "panel_por_maquina.csv";
        link.href = url
        link.click()
