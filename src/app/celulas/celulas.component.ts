...
export class CelulasComponent implements AfterContentInit {
  ...
  constructor( private router: Router, public dialog: MatDialog, private gestionBD: GestionApisService, private comunicacion: ComunicacionService, private observableMedia: ObservableMedia ) { 
    ...
    this.comunicacion.mostrarNota.emit("Células registrados en el sistema");
  ...
  llenarPantalla(orden) {
    ...
    //Recupera la tabla de celulas
      ...
      let consulta2 = "SELECT c.*, a.nombre AS tcreador, b.nombre AS tmodificador, (SELECT COUNT(maquinas.id) FROM maquinas WHERE maquinas.celula = c.id) AS tmaquinas FROM celulas AS c LEFT JOIN usuarios AS a ON c.creado = a.id LEFT JOIN usuarios AS b ON c.modificado = b.id ORDER BY" + cadOrden ;
  ...
  contarRegs() {
    ...
    if (this.celulas.length == 1) {
      this.comunicacion.mostrarEstado.emit(this.celulas.length + " célula" + filtrado);    
    }
    else if (this.celulas.length > 0 ) {
      this.comunicacion.mostrarEstado.emit(this.celulas.length + " células" + filtrado);
    }
  else{
  ...
  paro(id: number) {
    const dialogRef = this.dialog.open(DialogoCelulasComponent, {
  ...
  agregarParo() {
   ...
   descargar() {
    let exportCSV: string = "";
        ...
        exportCSV = "Listado de celulas registradas en el sistema: " + this.comunicacion.convertirFecha(1, '' , "yyyy/MM/dd hh:mm a") + "\r\n";
        exportCSV = exportCSV + '"ID de la celula (autonumerico)",';
        exportCSV = exportCSV + '"Nombre",';
        exportCSV = exportCSV + '"Referencia",';
        exportCSV = exportCSV + '"Prefijo",';
        exportCSV = exportCSV + '"# de maquinas asociadas",';
        ...
        this.celulas.forEach((elementos) => {
          ...
          exportCSV = exportCSV + '"' + elementos.tmaquinas + '",';
